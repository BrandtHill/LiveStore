defmodule LiveStoreWeb.UserAuth do
  use LiveStoreWeb, :verified_routes
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      LiveStoreWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      LiveStoreWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  def fetch_cart(%{assigns: %{current_user: nil}} = conn, _opts) do
    cart =
    ...
    conn
    |> put_resp_cookie(@cart_cookie, cart.id, @cart_options)
    |> assign(:cart, cart)
  ...
  the current_user:
      ...
      defmodule LiveStoreWeb.PageLive do
        use LiveStoreWeb, :live_view

        on_mount {LiveStoreWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{LiveStoreWeb.UserAuth, :require_authenticated}] do
  ...
  def on_mount(:require_sudo_mode, _params, session, socket) do
    ...
    if Accounts.sudo_mode?(socket.assigns.current_user, -30) do
      {:cont, socket}
    else
      socket =
        socket
